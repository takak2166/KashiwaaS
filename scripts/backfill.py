...
def main():
    ...
    logger.info(f"Backfilling messages from {start_date.strftime('%Y-%m-%d')} " f"to {end_date.strftime('%Y-%m-%d')}")
    ...
    while current_start < end_date:
        ...
        logger.info(
            ...
            f"{current_start.strftime('%Y-%m-%d')} to {batch_end.strftime('%Y-%m-%d')}"
        )

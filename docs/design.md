...
### 言語・フレームワーク
- プログラミング言語: Python 3.12以上（`pyproject.toml` の `requires-python` に準拠）
...
### 開発環境
- Docker Compose 構成（`docker-compose.yml` と一致。バージョンはリポジトリのファイルを正とする）:
  ```yaml
  version: '3.8'
  services:
    app:
...
- `.env` の読み込みは `apply_dotenv()`、環境からの組み立ては `load_config()` を **CLI**（`python -m src.cli`）および **ボット**（`python -m src.bot.kashiwaas`）の `main` で行う。モジュール import 時にグローバルな `config` シングルトンは持たない。
- `init_alerter(AppConfig)` でプロセス内のアラート送信（任意の Slack チャンネル）を初期化する。`ElasticsearchClient` / `SlackClient` / `KibanaCapture` は `AppConfig` の該当サブ設定をコンストラクタに渡す。
- 日次・週次の集計結果は `DailyStats` / `WeeklyStats`（`src/analysis/types.py`）で表現し、レポート整形はこれらを入力とする。
...
│   └── import_kibana_objects.py # Kibanaオブジェクトインポート
├── kibana/

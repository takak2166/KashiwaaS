...
- `CURSOR_MODEL`: Model ID (e.g. `composer-1.5`), or `Auto` / `default` / empty for API default. To see available IDs, call `GET https://api.cursor.com/v0/models` with your API key (Basic Auth).
...
- `CURSOR_MODEL`: モデルID（例: `composer-1.5`）。APIデフォルトを使う場合は `Auto` / `default` または未設定。利用可能なIDは `GET https://api.cursor.com/v0/models` をAPIキー（Basic認証）で呼ぶと確認できます。

...
- `CURSOR_MODEL`: Model ID (e.g. `composer-1.5`). If unset, the bot uses `composer-1.5` by default. To use the API default model instead, explicitly set this to `Auto`, `default`, or an empty string. To see available IDs, call `GET https://api.cursor.com/v0/models` with your API key (Basic Auth).
...
- `CURSOR_MODEL`: モデルID（例: `composer-1.5`）。未設定の場合、Botはデフォルトで `composer-1.5` を使います。APIデフォルトのモデルを使いたい場合は、明示的に `Auto` / `default` または空文字に設定してください。利用可能なIDは `GET https://api.cursor.com/v0/models` をAPIキー（Basic認証）で呼ぶと確認できます。
